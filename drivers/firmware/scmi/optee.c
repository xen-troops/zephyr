...
static int setup_dynamic_shmem(const struct device *tee_dev, struct scmi_optee_channel *channel)
{
	int ret;

	ret = tee_add_shm(tee_dev, NULL, OPTEE_MSG_NONCONTIG_PAGE_SIZE, SCMI_OPTEE_MAX_MSG_SIZE,
			  TEE_SHM_ALLOC | TEE_SHM_REGISTER, &channel->tee_shm);
	if (ret < 0) {
		LOG_ERR("shmem allocation failed: %d", ret);
		return ret;
	}

	memset(channel->tee_shm->addr, 0, SCMI_OPTEE_MAX_MSG_SIZE);
	channel->msg = channel->tee_shm->addr;
	channel->rx_len = SCMI_OPTEE_MAX_MSG_SIZE;
...
static int get_channel(const struct device *dev, struct scmi_optee_channel *channel)
{
				      ...
				      size_t msg_size)
{
	...
	param[1].a = 0;
	param[1].b = msg_size;
	param[1].c = (uint64_t)(uintptr_t)channel->tee_shm;

	param[2].attr = TEE_PARAM_ATTR_TYPE_MEMREF_OUTPUT;
	param[2].a = 0;
	param[2].b = SCMI_OPTEE_MAX_MSG_SIZE;
	param[2].c = (uint64_t)(uintptr_t)channel->tee_shm;
								   ...
								   struct scmi_message *msg)
{
	...
	/* 1. Extract response header */
	msg->hdr = layout->msg_hdr;

	/* 2. Calculate remaining payload length returned by OP-TEE */
	rx_payload_len = optee_chan->rx_len - sizeof(struct scmi_optee_msg_layout);

	/* 3. Copy response payload into scmi_message if buffer is provided */
...
out_unlock:
	/* Release channel lock acquired in scmi_optee_send_message */
	k_mutex_unlock(&optee_chan->lock);
