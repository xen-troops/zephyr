 ...
 *
 * preset      | minimum | maximum | energy_perf
 * ------------+---------+---------+----------------
...
#define XEN_SYSCTL_cpu_featureset_hvm_max  5
	...
	uint32_t nr_features; /*
			       * IN/OUT: Number of entries in/written to
...
struct livepatch_expectation {
	uint8_t enabled : 1;
	uint8_t len : 5;        /*
				 * Length of data up to LIVEPATCH_OPAQUE_SIZE
				 * (5 bits is enough for now)
				 */
	uint8_t rsv : 2;        /* Reserved. Zero value */
	uint8_t data[LIVEPATCH_OPAQUE_SIZE];	/*
						 * Same size as opaque[] buffer of
...
struct xen_sysctl_livepatch_list {
	uint32_t version;                       /*
						 * OUT: Hypervisor stamps value.
						 * If varies between calls, we are
						 * getting stale data.
						 */
	uint32_t idx;                           /* IN: Index into hypervisor list. */
	uint32_t nr;                            /*
						 * IN: How many status, name, and len
	...
	XEN_GUEST_HANDLE_64(xen_livepatch_status_t) status;  /*
							      * OUT. Must have enough
							      * space allocate for nr of them.
							      */
	XEN_GUEST_HANDLE_64(char) name;         /*
						 * OUT: Array of names. Each member
						 * may have an arbitrary length up to
						 * XEN_LIVEPATCH_NAME_SIZE bytes. Must have
						 * nr of them.
						 */
	XEN_GUEST_HANDLE_64(uint32) len;        /*
						 * OUT: Array of lengths of name's.
						 * Must have nr of them.
						 */
	XEN_GUEST_HANDLE_64(char) metadata;     /*
						 * OUT: Array of metadata strings. Each
						 * member may have an arbitrary length.
						 * Must have nr of them.
						 */
	XEN_GUEST_HANDLE_64(uint32) metadata_len;  /*
						    * OUT: Array of lengths of metadata's.
						    * Must have nr of them.
						    */
};
...
#define XEN_SYSCTL_cpu_policy_hvm_default  5
	...
	uint32_t nr_leaves;   /*
			       * IN/OUT: Number of leaves in/written to 'leaves',
			       * or the max number if 'leaves' is NULL.
			       */
	uint32_t nr_msrs;     /*
			       * IN/OUT: Number of MSRs in/written to 'msrs', or
